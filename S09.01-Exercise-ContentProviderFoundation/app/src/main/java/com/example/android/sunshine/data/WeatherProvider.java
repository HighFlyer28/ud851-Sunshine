...
//      Completed (9) Handle queries on both the weather and weather with date URI
        ...
        switch (match){
            case CODE_WEATHER:
                queryResult = db.query(WeatherContract.WeatherEntry.TABLE_NAME, projection, selection, selectionArgs,null, null, sortOrder);
                break;
            case CODE_WEATHER_WITH_DATE:
                String dateString = uri.getLastPathSegment();
                String[] newSelectionArgs = new String[]{dateString};
                queryResult = db.query(WeatherContract.WeatherEntry.TABLE_NAME,
                        projection,
